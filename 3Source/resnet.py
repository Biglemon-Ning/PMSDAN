...
class My_dense(nn.Module):
    def __init__(self, num_class=mfsan.num_classes):
        ...
        self.weight1 = nn.Parameter(torch.ones(1, num_class, requires_grad=True) * 0.4)     #混合主源域权重 0.4，0.2，0.2
        self.weight2 = nn.Parameter(torch.ones(1, num_class, requires_grad=True) * 0.2)
        self.weight3 = nn.Parameter(torch.ones(1, num_class, requires_grad=True) * 0.2)

    def forward(self, source1, source2, source3, source4):
        pred = (torch.mul(source1, self.weight2) + torch.mul(source2, (1 - self.weight1 - self.weight2 - self.weight3))
                + torch.mul(source3, self.weight1) + torch.mul(source4, self.weight3))
        return pred
...
class MFSAN(nn.Module):
    ...
    def forward(self, data_src, data_tgt=0, label_src=0, mark=1):
        ...
        if self.training == True:
            ...
            data_tgt_son4_mmd = data_tgt_son4.view(data_tgt_son4.size(0), -1)
            pred_tgt4 = self.cls_fc_son4(data_tgt_son4_mmd)
            ...
            data_src4 = self.avgpool(data_src4)
            data_src4_mmd = data_src4.view(data_src4.size(0), -1)
            pred4 = self.cls_fc_son4(data_src4_mmd)
            ...
            if mark == 1:
                ...
                l1_loss += torch.mean(torch.abs(torch.nn.functional.softmax(pred_tgt1, dim=1)
                                                - torch.nn.functional.softmax(pred_tgt4, dim=1)))
                l1_loss = l1_loss / 3
                ...
                mmd_loss += Loss(data_src1_mmd, data_tgt_son1_mmd, label_src, F.softmax(pred_tgt1, dim=1)) * 0.05
            ...
            if mark == 2:
                ...
                mmd_loss += Loss(data_src2_mmd, data_tgt_son2_mmd, label_src, F.softmax(pred_tgt2, dim=1)) * 0.05
            ...
            if mark == 3:
                l1_loss = torch.mean(torch.abs(torch.nn.functional.softmax(pred_tgt4, dim=1)
                                               - torch.nn.functional.softmax(pred_tgt1, dim=1)))
                l1_loss += torch.mean(torch.abs(torch.nn.functional.softmax(pred_tgt4, dim=1)
                                                - torch.nn.functional.softmax(pred_tgt2, dim=1)))
                l1_loss += torch.mean(torch.abs(torch.nn.functional.softmax(pred_tgt4, dim=1)
                ...
                cls_loss = F.nll_loss(F.log_softmax(pred4, dim=1), label_src)
                conditional_loss = torch.mul(-1 * F.softmax(pred_tgt4, dim=1), F.log_softmax(pred_tgt4, dim=1)).mean()

                # mmd加权
                weight_XX, weight_YY, weight_XY = Weight(pred4, pred_tgt4, label_src, F.softmax(pred_tgt4, dim=1))
                XX, YY, XY = mmd.mmd(data_src4_mmd, data_tgt_son4_mmd)
                mmd_loss += ((torch.mul(weight_XX, XX) + torch.mul(weight_YY, YY) - torch.mul(weight_XY, XY)).sum())

                mmd_loss += Loss(data_src4_mmd, data_tgt_son4_mmd, label_src, F.softmax(pred_tgt4, dim=1)) * 0.05

                mmd_loss = mmd_loss + conditional_loss * 0.1 + l1_loss

                return cls_loss, mmd_loss

            if mark == 4:  # 总体混在一起进行训练
                ...
                pred_src = self.mydense(pred1, pred2, pred3, pred4)
                pred_tgt = self.mydense(pred_tgt1, pred_tgt2, pred_tgt3, pred_tgt4)
                ...
                mmd_loss += Loss(data_src3_mmd, data_tgt_son3_mmd, label_src, F.softmax(pred_tgt, dim=1)) * 0.05
...
def Loss(out1, out2, label, pred):
    ...
    if num_inter != 0 and num_intra != 0:
        loss += torch.exp(0.1 * (intra_loss / num_intra)) + torch.exp(-0.1 * (inter_loss / num_inter))
    return loss